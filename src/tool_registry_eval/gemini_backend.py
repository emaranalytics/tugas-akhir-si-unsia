...
def _make_fn_decls(visible_tools: list[ToolDef]) -> list[types.FunctionDeclaration]:
    return [
...
def _extract_tool_call(resp: Any, tool_names: set[str]) -> str:
...
def _usage_numbers(resp: Any) -> tuple[int, int, int]:
...
def run_gemini_row(
    *,
    ...
    fn_decls = _make_fn_decls(visible_tools)
    ...
    selected_tool = _extract_tool_call(resp, tool_names)
    input_tokens, output_tokens, total_tokens = _usage_numbers(resp)
